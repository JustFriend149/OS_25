...
int main() {
	...
	//fd2 - файловые дескприпторы для передачи информации от ребенка к родителю
	size_t size;
	...
	if(result < 0) {
		...
		if(size != BYTES1 ) {
			printf("Родителю не удалось записать всю строку pipe\n");
			exit(-1);
		}
		if (close(fd1[1]) < 0) {
		...
		if (close(fd1[0]) < 0) {
			printf("Не удаплось закрыть входной поток процесса-ребенка\n");
			exit(-1);
		}
		size = write(fd2[1], "Информация для родительского процесса от ребенка", BYTES2);
		if(size != BYTES2) {
			printf("Ребенку не удалось записать всю строку pipe\n");
			exit(-1);
		}
		if (close(fd2[1]) < 0) {
